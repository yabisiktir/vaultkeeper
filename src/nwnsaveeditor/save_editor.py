...
edited ``.git`` swapped in — via the byte-faithful :mod:`nwnfile.formats.gff`
writer + :mod:`nwnfile.formats.erf_writer`, so everything untouched is
preserved exactly.
...
class SaveEditor:
    ...
    def set_property(
        ...
        ``iprp_*`` table (see :mod:`nwnfile.item_property_tables`), so the
    ...
    def player_spellbook(self) -> list[ClassSpellbook]:
        ...
        from nwnfile.character import class_name, is_base_class
        from nwnfile.character_reference import default_reference
